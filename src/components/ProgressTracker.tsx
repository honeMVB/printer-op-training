    ...
    <div className="bg-stone-900/90 border border-stone-800 rounded-2xl p-6 shadow-xl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-stone-800 pb-4 mb-4">
        <div>
          ...
          <p className="text-xs text-stone-400">Track module completions, quiz scores, and day-1 readiness badges.</p>
        </div>

        <div className="flex items-center gap-3">
          <div className="text-right">
            <span className="text-2xl font-black text-sky-400">{percentage}%</span>
      ...
      {/* Progress Bar */}
      <div className="w-full bg-stone-800 h-3 rounded-full overflow-hidden mb-6">
        <div
          className="bg-gradient-to-r from-sky-500 to-indigo-500 h-full transition-all duration-500 rounded-full"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
        <div className="bg-stone-800/60 p-3 rounded-xl border border-stone-700/50">
          <span className="text-xs text-stone-400 block mb-1">Modules Done</span>
          <span className="text-lg font-bold text-white">{completedModulesCount} / 7</span>
        </div>
        <div className="bg-stone-800/60 p-3 rounded-xl border border-stone-700/50">
          <span className="text-xs text-stone-400 block mb-1">Lessons Completed</span>
          <span className="text-lg font-bold text-white">{progress.completedLessons.length}</span>
        </div>
        <div className="bg-stone-800/60 p-3 rounded-xl border border-stone-700/50">
          <span className="text-xs text-stone-400 block mb-1">Quizzes Passed</span>
          <span className="text-lg font-bold text-teal-400">
            {Object.values(progress.quizScores).filter(s => s >= 70).length}
          </span>
        </div>
        <div className="bg-stone-800/60 p-3 rounded-xl border border-stone-700/50">
          <span className="text-xs text-stone-400 block mb-1">Certificate Status</span>
